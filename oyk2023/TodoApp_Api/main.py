...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # This is very permissive, fine for local testing, but restrict this in production.
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    if db_user:
        raise HTTPException(status_code=400, detail="You already exist duuude")
...
def read_user(user_id: int, db: Session = Depends(get_db)):
    ...
    if db_user is None:
        raise HTTPException(status_code=404, detail="You never existed duuuude")
    return db_user
...
def create_task_for_user(
    ...
    return crud.create_task_item(db=db, item=item, user_id=user_id)
